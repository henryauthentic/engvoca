...
class _SelectGoalPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final goals = [
      _GoalOption(words: 15, emoji: '🎯', desc: 'Nhẹ nhàng, phù hợp người bận rộn'),
      _GoalOption(words: 20, emoji: '⚡', desc: 'Cân bằng, phổ biến nhất'),
      _GoalOption(words: 30, emoji: '🔥', desc: 'Tập trung, tiến bộ nhanh'),
    ];
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Chọn số từ mới muốn học mỗi ngày để duy trì streak',
            style: TextStyle(
              fontSize: 14,
              color: isDark ? Colors.grey.shade400 : Colors.grey.shade600,
            ),
          ...
          const SizedBox(height: 28),
          ...goals.map((goal) {
            final isSelected = provider.dailyGoal == goal.words;
            return GestureDetector(
              onTap: () => provider.setDailyGoal(goal.words),
              child: AnimatedContainer(
                duration: const Duration(milliseconds: 250),
                margin: const EdgeInsets.only(bottom: 14),
...
class _GoalOption {
  ...
  final String emoji;
  final String desc;
  _GoalOption({required this.words, required this.emoji, required this.desc});
}
